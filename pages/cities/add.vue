                ...
                <template v-slot:item.2>
                  <v-card flat>
                    ...
                    <v-card-actions>
                      ...
                      <v-spacer></v-spacer>
                      <v-btn color="secondary" variant="outlined" @click="captureViewport">Use Viewport</v-btn>
                  ...
                  <v-card flat>
                    <v-card-text>
                      ...
                      <div v-else-if="importResult">
                        ...
                        <v-simple-table v-if="importResult.stops_imported > 0">
                          <thead><tr><th>Item</th><th>Count</th></tr></thead>
                          <tbody>
                            <tr><td>Bus Stops</td><td class="font-weight-bold">{{ importResult.stops_imported }}</td></tr>
                            <tr><td>Routes</td><td>{{ importResult.routes_imported || 0 }}</td></tr>
                            <tr><td>Import ID</td><td class="text-caption">{{ importResult.import_id }}</td></tr>
                          </tbody>
                        </v-simple-table>
                      </div>
                    </v-card-text>
                    <v-card-actions>
                      ...
                      <v-spacer></v-spacer>
                      <v-btn color="primary" @click="runImport" :loading="importing" :disabled="importing">Start Import</v-btn>
                    </v-card-actions>
                  </v-card>
                </template>
...
const steps = [
  { title: 'City Info', value: 1 },
  { title: 'Set Boundary', value: 2 },
  { title: 'Import Data', value: 3 },
...
async function initMap() {
  ...
  const bounds = L.rectangle([[5.55, -0.35], [5.70, -0.10]], {
    color: '#1976D2', weight: 2, fillOpacity: 0.1
  }).addTo(cityMap)
